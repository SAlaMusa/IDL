#!/bin/bash
#SBATCH --job-name=conf_crop_only
...
#SBATCH --mem=16G
#SBATCH --array=0-2%1
#SBATCH --output=/ocean/projects/cis260134p/mkipsang/IDL/logs/conf_crop_only_%A_%a.out
#SBATCH --error=/ocean/projects/cis260134p/mkipsang/IDL/logs/conf_crop_only_%A_%a.err
...
RUN_NAME="runs/crop_only_seed${SEED}"
OUT="results/confirmatory/crop_only_seed${SEED}.csv"
...
if [ -f "$FINAL_CKPT" ]; then
    ...
    if [ -n "$LAST_CKPT" ]; then
        ...
        python run.py --config configs/crop_only.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME --resume "$LAST_CKPT"
    else
        echo "==> Starting crop_only seed=$SEED from scratch"
        python run.py --config configs/crop_only.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME
    fi
fi
