...
#SBATCH --mem=16G
#SBATCH --array=0-2%1
#SBATCH --output=/ocean/projects/cis260134p/mkipsang/IDL/logs/conf_pair_jg_%A_%a.out
#SBATCH --error=/ocean/projects/cis260134p/mkipsang/IDL/logs/conf_pair_jg_%A_%a.err
...
RUN_NAME="runs/pair_jitter_grayscale_seed${SEED}"
OUT="results/confirmatory/pair_jitter_grayscale_seed${SEED}.csv"
...
if [ -f "$FINAL_CKPT" ]; then
    ...
    if [ -n "$LAST_CKPT" ]; then
        ...
        python run.py --config configs/pair_jitter_grayscale.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME --resume "$LAST_CKPT"
    else
        echo "==> Starting pair_jitter_grayscale seed=$SEED from scratch"
        python run.py --config configs/pair_jitter_grayscale.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME
    fi
fi
