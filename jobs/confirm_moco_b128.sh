...
RUN_NAME="runs/moco_b128_seed${SEED}"
OUT="results/confirmatory/moco_b128_seed${SEED}.csv"
...
if [ -f "$FINAL_CKPT" ]; then
    ...
    if [ -n "$LAST_CKPT" ]; then
        ...
        python run_moco.py --config configs/moco_b128.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME --resume "$LAST_CKPT"
    else
        echo "==> Starting MoCo v2 b128 seed=$SEED from scratch"
        python run_moco.py --config configs/moco_b128.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME
    fi
fi
