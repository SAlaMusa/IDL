...
RUN_NAME="runs/ablation_no_crop_seed${SEED}"
OUT="results/confirmatory/ablation_no_crop_seed${SEED}.csv"
...
if [ -f "$FINAL_CKPT" ]; then
    ...
    if [ -n "$LAST_CKPT" ]; then
        ...
        python run.py --config configs/ablation_no_crop.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME --resume "$LAST_CKPT"
    else
        echo "==> Starting ablation_no_crop seed=$SEED from scratch"
        python run.py --config configs/ablation_no_crop.yaml --seed $SEED --epochs 800 --run-name $RUN_NAME
    fi
fi
